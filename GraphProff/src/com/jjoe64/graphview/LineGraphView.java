	...
	public void drawSeries(Canvas canvas, GraphViewData[] values, float graphwidth, float graphheight, 
		...
		for (int i = 0; i < values.length; i++) {
			...
			if (i > 0) {
				...
				canvas.drawCircle(endX, endY, 3, new Paint(paint));
				canvas.drawCircle(endX, endY, 2, new Paint(paint));
				
				
			}
			else
			{
				float startX = (float) x + (horstart + 1);
				float startY = (float) (border - y) + graphheight;
								
				canvas.drawCircle(startX, startY, 3, new Paint(paint));
				canvas.drawCircle(startX, startY, 2, new Paint(paint));
			}
			lastEndY = y;
			lastEndX = x;
		}
