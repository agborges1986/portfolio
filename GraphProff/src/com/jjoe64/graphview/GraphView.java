...
//			
		...
		protected void onDraw(Canvas canvas) {
			...
			// vertical labels
				...
				int vers = verlabels.length - 1;
				for (int i = 0; i < verlabels.length; i++) {
					float y = ((graphheight / vers) * (verlabels.length-1-i)) + border;
...
//	private String yNameLabel;
//	private String xNameLabel;
	...
	public final View viewVerLabels;
	public ScaleGestureDetector scaleDetector;
	public boolean scalable;
	public final NumberFormat[] numberformatter = new NumberFormat[2];
	public final List<GraphViewSeries> graphSeries;
	public boolean showLegend = false;
	public float legendWidth = 120;
	public LegendAlign legendAlign = LegendAlign.MIDDLE;
	public boolean manualYAxis;
	public double manualMaxYValue;
	public double manualMinYValue;
	public boolean manualXAxis;
	public double manualMaxXValue;
	public double manualMinXValue;
	public GraphViewStyle graphViewStyle;
	...
	protected String formatLabel(double value, boolean isValueX) {
		...
		if (numberformatter[i] == null) {
			...
			if (highestvalue - lowestvalue < 0.1) {
				...
				numberformatter[i].setMinimumIntegerDigits(4);
			}
		}
	...
	protected String[] generateHorlabels(float graphwidth) {
		int numLabels = getNumXLabels(graphwidth);
...
//		TODO doing for ariel for one case. No matters
//		if(xNameLabel==null)
//			labels[numLabels]="Eje X";
//		else
//			labels[numLabels]=xNameLabel;
		return labels;
	}

	protected synchronized String[] generateVerlabels(float graphheight) {
		int numLabels = getNumYLabels(graphheight);
		String[] labels = new String[numLabels+1];
		double min = getMinY();
		double max = getMaxY();
		if (max == min) {
		...
		for (int i=0; i<=numLabels; i++) {
			labels[i] = formatLabel(min + ((max-min)*i/numLabels), false);
		}
//		TODO doing for ariel for one case. No matters
//		if(yNameLabel==null)
//			labels[numLabels]="Eje Y";
//		else
//			labels[numLabels]=yNameLabel;
		return labels;
	}
	
	public int getNumYLabels(float graphheight){
		return (int) (graphheight/GraphViewConfig.HORIZONTAL_LABEL_HEIGHT);
	}
	public int getNumXLabels(float graphwidth){
		return (int) (graphwidth/GraphViewConfig.VERTICAL_LABEL_WIDTH);
	}
	/**
	 ...
	 * 
	 */
	public void setyNameLabel(String yname){
//		if(yNameLabel==null && verlabels!=null)
//			verlabels[verlabels.length-1]=yname;
//		this.yNameLabel=yname;
	}
	/**
	 ...
	 * 
	 */
	public void setXNameLabel(String xname){
//		if(xNameLabel==null && horlabels!=null)
//			horlabels[horlabels.length-1]=xname;
//		this.xNameLabel=xname;
	}
...
//		This is for permit more space in Canvas for set axis Y Name
		return largest;
	}
...
//		String [] tmp =new String[horlabels.length+1];
//		for(int i=0;i<horlabels.length;i++)
//		{
//			tmp[i]=horlabels[i];
//		}
//		if (xNameLabel==null)
//			tmp[horlabels.length]="Eje X";
//		else
//			tmp[horlabels.length]=xNameLabel;
//		
//			this.horlabels = tmp;
	...
	/**
	 * set manual X axis limit
	 * @param max
	 * @param min
	 */
	public void setManualXAxisBounds(double max, double min) {
		manualMaxXValue = max;
		manualMinXValue = min;
		manualYAxis = true;
	}
...
//		String [] tmp =new String[verlabels.length+1];
//		for(int i=0;i<verlabels.length;i++)
//		{
//			tmp[i]=verlabels[i];
//		}
//		if (yNameLabel==null)
//			tmp[verlabels.length]="Eje Y";
//		else
//			tmp[verlabels.length]=yNameLabel;
//		
//			this.verlabels = tmp;
	...
	public boolean isManualXAxis() {
		return manualXAxis;
	}

	public void setManualXAxis(boolean manualXAxis) {
		this.manualXAxis = manualXAxis;
	}
